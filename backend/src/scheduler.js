// Core schedule generation ΓÇö ported from the browser app
...
function generateSchedule({ teachers, attendants, pairs, venues, exams, ownSubjectRule = false }) {
  ...
  if (unknownVenues.size) warnings.push(`Unknown venue(s): ${[...unknownVenues].join(', ')} ΓÇö defaulted to 1 pair.`);
  if (noDurationRows.size) warnings.push(`Could not determine duration for exam row(s) ${[...noDurationRows].join(', ')} ΓÇö defaulted to 60 min.`);
